...
if [ -f "$arquivoWorkspace" ]; then
    ...
    fnames=(exportar_professores exportar_equipe_escolar exportar_turmas workspace)
else
    # fnames=(exportar_alunos exportar_professores exportar_equipe_escolar exportar_turmas)
    fnames=(exportar_professores exportar_equipe_escolar exportar_turmas)
fi
...
# https://matricula.juazeiro.ba.gov.br/relatorios/exportar_alunos_aee_deficiencias_csv/2026?chave=SIEJaoDahFeh21
# https://matricula.juazeiro.ba.gov.br/relatorios/disciplinas_gestao_escolar_csv?chave=SIEJaoDahFeh21
# https://matricula.juazeiro.ba.gov.br/relatorios/exportar_deficiencias_csv/2026?chave=SIEJaoDahFeh21
# https://matricula.juazeiro.ba.gov.br/relatorios/exportar_alunos_movimentados_csv/2026?chave=SIEJaoDahFeh21
# https://matricula.juazeiro.ba.gov.br/relatorios/exportar_alunos_csv/2026?chave=SIEJaoDahFeh21

    # wget -O 0adm_exportar_alunos.csv --no-check-certificate https://matricula.juazeiro.ba.gov.br/relatorios/exportar_alunos_csv/$ano?chave=SIEJaoDahFeh21
    ...
    # alunos movimentados
    wget -O 0adm_exportar_alunos_movimentados.csv --no-check-certificate https://matricula.juazeiro.ba.gov.br/relatorios/exportar_alunos_movimentados_csv/$ano?chave=SIEJaoDahFeh21
    codigo_exportar_alunos_movimentados=$?
    echo "Código $codigo_exportar_alunos_movimentados"
    ...
    wget -O 0adm_exportar_deficiencias.csv --no-check-certificate https://matricula.juazeiro.ba.gov.br/relatorios/exportar_deficiencias_csv/$ano?chave=SIEJaoDahFeh21
    codigo_exportar_deficiencias=$?
    echo "Código $codigo_exportar_deficiencias"


    wget -O 0adm_exportar_alunos_aee_deficiencias.csv --no-check-certificate https://matricula.juazeiro.ba.gov.br/relatorios/exportar_alunos_aee_deficiencias_csv/$ano?chave=SIEJaoDahFeh21
    codigo_exportar_alunos_aee_dificiencias=$?
    echo "Código $codigo_exportar_alunos_aee_dificiencias"
    ...
    if [[ $codigo_exportar_equipe_escolar != 0 || $codigo_exportar_professores != 0 || $codigo_exportar_turmas != 0 || $codigo_exportar_rotas != 0 || $codigo_exportar_rotas_alunos != 0 || $codigo_exportar_alunos_autistas != 0 || $codigo_exportar_deficiencias != 0 || $codigo_exportar_alunos_aee_dificiencias != 0 ]]; then
    echo "Ocorreram erros nos downloads dos arquivos CSVs."
        if [[ $codigo_exportar_equipe_escolar != 0 ]]; then
            echo "Erro ao baixar o arquivo exportar_equipe_escolar.csv"
        fi
        if [[ $codigo_exportar_professores != 0 ]]; then
            echo "Erro ao baixar o arquivo exportar_professores.csv"
        fi
        if [[ $codigo_exportar_turmas != 0 ]]; then
            echo "Erro ao baixar o arquivo exportar_turmas.csv"
        fi
        if [[ $codigo_exportar_rotas != 0 ]]; then
            echo "Erro ao baixar o arquivo 0adm_exportar_rotas.csv"
        fi
        if [[ $codigo_exportar_rotas_alunos != 0 ]]; then
            echo "Erro ao baixar o arquivo 0adm_exportar_rota_aluno.csv"
        fi
        if [[ $codigo_exportar_alunos_autistas != 0 ]]; then
            echo "Erro ao baixar o arquivo 0adm_exportar_alunos_autistas.csv"
        fi
        if [[ $codigo_exportar_deficiencias != 0 ]]; then
            echo "Erro ao baixar o arquivo exportar_deficiencias.csv"
        fi
        if [[ $codigo_exportar_alunos_aee_dificiencias != 0 ]]; then
            echo "Erro ao baixar o arquivo exportar_alunos_aee_dificiencias.csv"
        fi
        if [[ $codigo_exportar_alunos_movimentados != 0 ]]; then
            echo "Erro ao baixar o arquivo 0adm_exportar_alunos_movimentados.csv"
        fi
     exit 127
    fi
...
if [ $1 == 'local' ]; then
    ...
    # /usr/bin/php "$dirArtisnan"artisan db:seed --class=ExportarCsvAlunos --force
    /usr/bin/php "$dirArtisnan"artisan db:seed --class=ExportarCsvRotaAluno --force
    /usr/bin/php "$dirArtisnan"artisan db:seed --class=ExportarCsvRota --force
    /usr/bin/php "$dirArtisnan"artisan db:seed --class=ExportarCsvAlunoAutista --force
    /usr/bin/php "$dirArtisnan"artisan db:seed --class=ExportarCsvDeficiencias --force
    /usr/bin/php "$dirArtisnan"artisan db:seed --class=ExportarCsvDeficienciaAluno --force
    /usr/bin/php "$dirArtisnan"artisan db:seed --class=ExportarCsvAlunosMovimentados --force
elif [ $1 == 'online' ]; then
    ...
    # /usr/local/bin/php "$dirArtisnan"artisan db:seed --class=ExportarCsvAlunos --force --no-interaction
    /usr/local/bin/php "$dirArtisnan"artisan db:seed --class=ExportarCsvRotaAluno --force --no-interaction
    /usr/local/bin/php "$dirArtisnan"artisan db:seed --class=ExportarCsvRota --force --no-interaction
    /usr/local/bin/php "$dirArtisnan"artisan db:seed --class=ExportarCsvAlunoAutista --force --no-interaction
    /usr/local/bin/php "$dirArtisnan"artisan db:seed --class=ExportarCsvDeficiencias --force --no-interaction
    /usr/local/bin/php "$dirArtisnan"artisan db:seed --class=ExportarCsvDeficienciaAluno --force --no-interaction
    /usr/local/bin/php "$dirArtisnan"artisan db:seed --class=ExportarCsvAlunosMovimentados --force --no-interaction
fi
...
# CREATE INDEX IF NOT EXISTS idx_aluno__id ON adm_0adm_exportar_alunos(aluno__id);
# CREATE INDEX IF NOT EXISTS idx_aluno__responsavel__cpf ON adm_0adm_exportar_alunos(responsavel__cpf);
# CREATE INDEX IF NOT EXISTS idx_escola__inep ON adm_0adm_exportar_alunos(escola__inep);
# CREATE INDEX IF NOT EXISTS idx_escola_id ON adm_0adm_exportar_alunos(escola__id);
# CREATE INDEX IF NOT EXISTS idx_sala_id ON adm_0adm_exportar_alunos(sala__id);
# CREATE INDEX IF NOT EXISTS idx_turma_id ON adm_0adm_exportar_alunos(turma__id);
# CREATE INDEX IF NOT EXISTS idx_turma_nome ON adm_0adm_exportar_alunos(turma__nome);
# CREATE INDEX IF NOT EXISTS idx_turno_id ON adm_0adm_exportar_alunos(turno__id);
# CREATE INDEX IF NOT EXISTS idx_aluno_cor ON adm_0adm_exportar_alunos(aluno__cor);
...
############## Atualiza IDs adm_papel_id e Cargos/Funções (adm_funcao_id)
SELECT '' as 'Atualiza IDs adm_papel_id e Cargos/Funções';
UPDATE 0adm_exportar_equipe_escolar AS t1

# atualiza coluna adm_papel_id (vem da tabela adm_roles) no sistema Educajua Pedagógico
SET t1.adm_papel_id = 
CASE 
...
WHEN funcao__id = 44 THEN 63 #auxiliar de creche
WHEN funcao__id = 85 THEN 64 #Mae Social
END,

########## atualiza coluna adm_funcao_id (vem da tabela adm_cargos) na escola
...
WHEN funcao__id = 42 THEN 4 #Auxiliar de Atendimento Educacional Especializado
...
WHEN funcao__id = 44 THEN 5 #auxiliar de creche
WHEN funcao__id = 85 THEN 9 #Mae Social
WHEN funcao__id = 230 THEN 29 #Professor Educação Infantil - Apoio Pedagógico
WHEN funcao__id = 231 THEN 30 #Professor Anos Iniciais - Apoio Pedagógico
WHEN funcao__id = 232 THEN 32 #Professor Anos Finais - Apoio Pedagógico
END
;
...
# Ajusta CPF
# SELECT '' as 'AJUSTE DE CPF adm_0adm_exportar_alunos';
# UPDATE adm_0adm_exportar_alunos
# SET
...
# -- cria colunas
# ALTER TABLE adm_0adm_exportar_alunos
...
# UPDATE adm_0adm_exportar_alunos AS t1
# SET t1.status = REPLACE(REPLACE(REPLACE(t1.status, CHAR(13, 10), ''), '\n', ''), '\r', '');

# UPDATE adm_0adm_exportar_alunos AS t1
# INNER JOIN adm_escolas AS t2 ON t1.escola__id = t2.idnumber
# SET t1.adm_escola_id = t2.id;
# ALTER TABLE adm_0adm_exportar_alunos ADD INDEX IF NOT EXISTS adm_escola_id_index (adm_escola_id);

# UPDATE adm_0adm_exportar_alunos AS t1
# INNER JOIN adm_turmas AS t2 ON t1.turma__id = t2.idnumber
# SET t1.adm_turma_id = t2.id;
# ALTER TABLE adm_0adm_exportar_alunos ADD INDEX IF NOT EXISTS adm_turma_id_index (adm_turma_id);
...
# SET t1.adm_user_id = t2.id;
# ALTER TABLE adm_0adm_exportar_alunos ADD INDEX IF NOT EXISTS adm_user_id_index (adm_user_id);
...
# INNER JOIN 0adm_exportar_turmas AS t3 ON t3.turma__id = t1.turma__id
# SET t1.adm_turma_id = t2.id
# where t3.multi like '%True%';
# ALTER TABLE adm_0adm_exportar_alunos ADD INDEX IF NOT EXISTS adm_turma_id_index (adm_turma_id);

# UPDATE adm_0adm_exportar_alunos AS t1
# INNER JOIN adm_series AS t2 ON t1.serie__id = t2.idnumber
# SET t1.adm_serie_id = t2.id;
# ALTER TABLE adm_0adm_exportar_alunos ADD INDEX IF NOT EXISTS adm_serie_id_index (adm_serie_id);

# UPDATE adm_0adm_exportar_alunos AS t1
# INNER JOIN adm_etapas AS t2 ON t1.etapa__id = t2.idnumber
# SET t1.adm_etapa_id = t2.id;
# ALTER TABLE adm_0adm_exportar_alunos ADD INDEX IF NOT EXISTS adm_etapa_id_index (adm_etapa_id);

# UPDATE adm_0adm_exportar_alunos AS t1
# INNER JOIN adm_racas_cores AS t2 ON t1.aluno__cor = t2.idnumber
# SET t1.adm_cor_raca_id = t2.id;
# ALTER TABLE adm_0adm_exportar_alunos ADD INDEX IF NOT EXISTS adm_cor_raca_id_index (adm_cor_raca_id);
...
# ALTER TABLE adm_0adm_exportar_alunos ADD INDEX IF NOT EXISTS adm_statusmatricula_id_index (adm_statusmatricula_id);

## Turno ID
# UPDATE adm_0adm_exportar_alunos AS t1
# INNER JOIN adm_turnos AS t2 ON t1.turno__id = t2.idnumber
# SET t1.adm_turno_id = t2.id;
# ALTER TABLE adm_0adm_exportar_alunos ADD INDEX IF NOT EXISTS adm_turno_id_index (adm_turno_id);
...
# case when a.aluno__utiliza_nome_social = 1 then 1 else 0 end,
...
# case when a.aluno__possui_deficiencia = 1 then 1 else 0 end,
# adm_cor_raca_id
# from adm_0adm_exportar_alunos a
# where NOT EXISTS
...
#     case when a.utiliza_transporte = 1 then 1 else 0 end,
#     0 as tipo_matricula,
#     concat('', now()),
#     now()
# from adm_0adm_exportar_alunos a
...
# LEFT JOIN adm_0adm_exportar_alunos exp_a ON exp_a.adm_user_id = a.user_id AND exp_a.adm_turma_id = a.turma_id and a.ativo = 1
...
#         exp_tu.adm_turma_id, 
#         s.id AS serie_id
#     FROM adm_0adm_exportar_alunos exp_a
...
AND role_id = 10    # Permissão de professor
And role_id = 28;   # Permissão de professor comunicante

############################## Adiciona/Remove permissao de professores

SELECT '' as 'Remove permissoes de professores';
DELETE p FROM adm_model_has_roles p
WHERE NOT EXISTS (
    SELECT 1 FROM \`0adm_exportar_professores\` exp_p
    WHERE exp_p.adm_user_id = p.model_id
...
############################## Adiciona Vinculo Unidade para os professores como Comunicantes do Caic
INSERT IGNORE INTO adm_permissoes (role_id, user_id, escola_id, created_at, updated_at)
SELECT 28, adm_user_id, adm_escola_id, now(), now() FROM 0adm_exportar_professores exp_p
WHERE NOT EXISTS (
    SELECT 1 FROM adm_permissoes p
    WHERE exp_p.adm_user_id = p.user_id
    AND exp_p.adm_escola_id = p.escola_id
    AND p.role_id = 28
    AND exp_p.adm_escola_id = 54
)
AND adm_user_id is NOT null
and adm_escola_id = 54
group by adm_escola_id, adm_user_id;
...
############################## adiciona permissao de professores como Comunicantes do Caic
INSERT IGNORE INTO adm_model_has_roles (role_id, model_type, model_id)
SELECT 28, 'App\\\Models\\\User', adm_user_id FROM 0adm_exportar_professores exp_p
where NOT EXISTS
    (SELECT 1 FROM adm_model_has_roles mhr
     where mhr.role_id = 28
     and mhr.model_id =  exp_p.adm_user_id
     and exp_p.adm_escola_id = 54)
and exp_p.adm_user_id is NOT null
and exp_p.adm_escola_id = 54
...
SELECT '' as 'Alocação de professores';
...
#####################################################
SELECT '' as 'atualiza IDs de turmas do portal e pedagógico';
...
SELECT '' as 'Atualização de adm_turno_id na tabela adm_0adm_exportar_rotas';
UPDATE IGNORE adm_0adm_exportar_rotas
SET adm_turno_id = (
    SELECT id FROM adm_turnos WHERE idnumber = adm_0adm_exportar_rotas.turno_id
);

SELECT '' as 'Atualização de adm_calendario_id na tabela adm_0adm_exportar_rotas';
UPDATE IGNORE adm_0adm_exportar_rotas er
SET er.adm_calendario_id = (
    SELECT c.id FROM adm_calendarios c WHERE c.ano = er.ano_letivo
)
WHERE er.ano_letivo <> '';

############# atualiza rotas existentes
UPDATE adm_rotas r
JOIN adm_0adm_exportar_rotas er ON er.rota_id = r.idnumber
SET
    r.nome_antigo = er.rota_nome_antigo,
    r.nome = er.rota_nome,
    r.turno_id = er.adm_turno_id,
    r.capacidade = er.capacidade,
    r.motorista_nome = er.motorista_nome,
    r.updated_at = now()
WHERE r.idnumber = er.rota_id;
...
# and NOT EXISTS (SELECT 1 FROM adm_0adm_exportar_alunos exp where exp.adm_user_id = m.user_id);

# # Atualiza dados de matriculas
# SELECT '' as 'Atualiza dados de matriculas';

# UPDATE adm_matriculas m
# join adm_0adm_exportar_alunos a on a.adm_user_id = m.user_id
...
# m.transporte_publico = case when a.utiliza_transporte = 1 then 1 else 0 end,
# m.updated_at = now()
...
# UPDATE adm_users u
# join adm_0adm_exportar_alunos a on a.adm_user_id = u.id
# SET
# u.ne = case when a.aluno__possui_deficiencia = 1 then 1 else 0 end,
# u.name = a.aluno__nome,
# u.nome_social = a.aluno__nome_social,
# u.usar_nome_social = case when a.aluno__utiliza_nome_social = 1 then 1 else 0 end,
# u.nome_responsavel = a.responsavel__nome,
# u.bairro = a.aluno__bairro__nome,
# u.endereco = a.aluno__endereco__endereco,
...
#         WHERE eaa.idnumber = u.idnumber
#     ) THEN true 
#     ELSE false 
# END;

# -- 1. Marca todos como false
# UPDATE adm_users 
# SET autista = false;

# -- 2. Marca os autistas como true
# UPDATE adm_users u
# INNER JOIN adm_0adm_exportar_alunos_autistas eaa 
#         ON eaa.idnumber = u.idnumber
# SET u.autista = true;
